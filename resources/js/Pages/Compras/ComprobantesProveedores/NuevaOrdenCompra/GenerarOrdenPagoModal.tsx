...
export default function GenerarOrdenPagoModal({
  ...
  const [pagosUnicos, setPagosUnicos] = useState<Pago[]>([
    { metodo: "", moneda: Number(monedaOrden), importe: totalOrden, fecha: "" }
  ])
  ...
  const allComplete = plan === "unico"
    ? pagosUnicos.every(isPagoCompleto)
    : pagosCuotas.flat().every(isPagoCompleto)

  const calcTotal = (pagos: Pago[]) => pagos.reduce((acc, p) => acc + p.importe, 0)
  const calcTotalPorCuota = (pagoList: Pago[]) => pagoList.reduce((acc, p) => acc + p.importe, 0)
  ...
  const getImporteFaltante = (actualTotal: number) => {
    const faltante = totalOrden - actualTotal
  ...
  const handleFinalSubmit = () => {
    ...
    const dataToSend =
        ...
        ? {
            ...
            pagos: pagosUnicos.map((p) => ({
                ...
                cbu_pago: p.cbuProveedorId ? Number(p.cbuProveedorId) : null,
                usuario_id: 1,
            })),
            }
        : {
    ...
    if (totalPagos > totalOrden) {
        alert("La suma de los importes supera el total de la orden");
        return;
    }

    router.post("/compras/ordenes-compras/ordenes-pagos", dataToSend, {
        onSuccess: (page) => {
        onSubmit(page.props);
        onClose();
        },
        ...
        alert("Ocurrió un error al guardar la orden de pago. Por favor, intentá nuevamente.");
        },
    });
    };
          ...
          <div className="col-span-6 grid grid-cols-1 gap-2">
            ...
            <div className="col-span-1">
              <select
                ...
                onChange={(e) => updateMultipleFields({ cbuProveedorId: Number(e.target.value) })}
              >
              ...
              <div>
                ...
                {pagosUnicos.map((p, idx) => (
                  <div key={idx} className="grid grid-cols-6 gap-2 mb-2 items-center">
                    <select value={p.metodo} onChange={(e) => { const updated = [...pagosUnicos];
                ...
                <div className="text-sm text-gray-600 mt-2">Total cargado: {totalPagosUnicos} / {totalOrden}</div>
              </div>
            ) : (
                ...
                {pagosCuotas.map((pagoList, cuotaIdx) => (
                  <TabsContent key={cuotaIdx} value={String(cuotaIdx)}>
                    {Array.isArray(pagoList) ? pagoList.map((p, idx) => (
                      <div key={idx} className="grid grid-cols-6 gap-2 mb-2 items-center">
                        <select value={p.metodo} onChange={(e) => { const updated = [...pagosCuotas]; updated[cuotaIdx][idx].metodo = e.target.value; setPagosCuotas(updated); }}>
                          <option value="">Método</option>
                          <option value="Transferencia">Transferencia</option>
                          <option value="Efectivo">Efectivo</option>
                          <option value="Tarjeta">Tarjeta</option>
                    ...
                    <div className="text-sm text-gray-600 mt-2">Total de esta cuota: {calcTotalPorCuota(pagoList)}</div>
                  </TabsContent>
                ))}

                <div className="text-sm text-gray-600 mt-2">Total cargado: {totalPagosCuotas} / {totalOrden}</div>
              </Tabs>
            )}
