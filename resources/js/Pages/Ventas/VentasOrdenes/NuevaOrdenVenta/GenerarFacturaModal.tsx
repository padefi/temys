...
export default function GenerarFacturaModal({
    ...
    const { auth, impuestos, co_cuentas, productos: productosDisponibles = [], } = usePage().props
    ...
    // 🆕 Nuevo estado para el próximo número de anticipo
    const [proximoNumeroAnticipo, setProximoNumeroAnticipo] = useState<number | null>(null)
    ...
    // 🆕 Nuevo useEffect para cargar el próximo número de anticipo
    useEffect(() => {
        // Solo cargar cuando el modal se abre y si aún no se ha cargado el número
        if (open && tiposComprobante.length > 0 && proximoNumeroAnticipo === null) {
            const anticipo = tiposComprobante.find(tc => tc.nombre.toLowerCase() === 'anticipo')
            if (anticipo) {
                ...
                axios.get(`/compras/ordenes-compras/comprobantes-proveedores/proximo-numero-anticipo`)
                    ...
                    .catch(err => {
                        console.error("Error cargando próximo número de anticipo", err)
                        // Si falla, permitimos la entrada manual o un reintento.
                    })
            }
        }
    }, [open, tiposComprobante, proximoNumeroAnticipo])
    ...
    useEffect(() => {
        ...
        const todosValidos = productosLocal.length > 0 && productosLocal.every(p => p.producto_id > 0 && p.descripcion.trim() !== '' && p.cantidad > 0 && p.precio_unitario > 0)
    ...
    // 🔄 Modificado para aceptar números y ser más robusto
    const padLeft = (value: string | number, length: number) => {
        if (!value && value !== 0) return "";
        return String(value).padStart(length, "0");
    };

    // 🆕 Handler para Tipo de Comprobante con la lógica de "Anticipo"
    const handleChangeTipoComprobante = (value: string) => {
        setTipoComprobante(value)

        const isAnticipo = value.toLowerCase() === 'anticipo'

        if (isAnticipo) {
            // 1. Set Punto de Venta a '0001' (formato de 4 dígitos)
            setPuntoVenta('0001')

            // 2. Set Numero de Factura al próximo número secuencial (formato de 8 dígitos)
            if (proximoNumeroAnticipo !== null) {
                setNumeroFactura(padLeft(proximoNumeroAnticipo, 8))
            } else {
                toast.warning("El próximo número de anticipo aún no ha sido cargado. Por favor, inténtelo de nuevo o ingrese manualmente.")
                setNumeroFactura('')
            }
        } else {
            // Si no es Anticipo, borra los campos para la entrada manual
            setPuntoVenta('')
            setNumeroFactura('')
        }
    }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const backendMessage = error.response?.data?.error || error.response?.data?.message || "Error al guardar la factura.";
    ...
    const isAnticipoSelected = tipoComprobante.toLowerCase() === 'anticipo'
                ...
                <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-2xl w-full max-w-7xl shadow-lg max-h-[90vh] overflow-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <Input list="condicionesVenta" value={condicionVenta} onChange={e => setCondicionVenta(e.target.value)} />
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <Input
                                    ...
                                    placeholder="Pto Vta"
                                    value={puntoVenta}
                                    // 🔄 Deshabilitar si es Anticipo
                                    disabled={isAnticipoSelected}
                        ...
                        <div>
                            ...
                            <Input
                                ...
                                onChange={e => handleChangeTipoComprobante(e.target.value)}
                            />
                    ...
                    <Tabs defaultValue="productos" className="w-full">
                        <TabsContent value="productos">
                                    ...
                                    <DropdownMenuContent>
                                        {Object.entries(columnasVisibles).map(([col, visible]) => (
                                            <DropdownMenuCheckboxItem
                                                key={col}
                                                checked={visible}
                                                onCheckedChange={() => setColumnasVisibles(prev => ({ ...prev, [col]: !prev[col as keyof typeof prev] }))
                                                }
                                            >
                                    ...
                                    {productosLocal.map((producto, index) => {
                                        ...
                                        return (
                                            <TableRow key={producto.id}>
                                                <TableCell>
                                                    <input list="sugerencias-productos" type="text"
                                                ...
                                                {columnasVisibles.entrega_esperada && (
                                                    <TableCell>
                                                        <input
                                                            ...
                                                            type="date"
                                                            value={
                                                                producto.entrega_esperada ? new Date(producto.entrega_esperada).toISOString().split("T")[0] : ""
                                                            }
                                                            onChange={(e) => handleChangeCampo(index, "entrega_esperada", new Date(e.target.value))
                                                            }
                                                            className="w-full border px-2 py-1 rounded"
                                                        />
                                                    </TableCell>
                                                    ...
                                                    <TableCell>
                                                        <Popover>
                                                            <PopoverTrigger asChild>
                                                                <Button
                                                                    variant="outline"
                                                                    className={`w-full justify-start ${!producto.co_cuenta_id ? "border-red-500 text-red-600" : ""
                                                                        }`}
                                                                >
                                                                    {producto.co_cuenta_id ? `${co_cuentas.find(c => c.id === producto.co_cuenta_id)?.codigo} - ${co_cuentas.find(c => c.id === producto.co_cuenta_id)?.descripcion}` : "Seleccionar cuenta"}
                                                                </Button>
                                                            </PopoverTrigger>
                                                        ...
                                                        <Popover>
                                                            <PopoverTrigger asChild>
                                                                <Button variant="outline" className="w-full justify-start">
                                                                    {producto.impuestos_seleccionados.length ? impuestos
                                                                        .filter(i => producto.impuestos_seleccionados.includes(i.id))
                                                                        .map(i => i.descripcion)
                                                                        .join(', ') : 'Seleccionar impuestos'}
                                                                </Button>
                                                            </PopoverTrigger>
                                                ...
                                                <TableCell>
                                                    <Button variant="ghost" size="sm" onClick={() => handleEliminarProducto(index)} className="text-red-500 hover:text-red-700"
                                                    >
                                                        🗑
                                                    </Button>
                                                </TableCell>
                            ...
                            <Input type="file" multiple accept=".jpg,.jpeg,.png,.pdf,.doc,.docx" onChange={handleArchivosChange} />
                            {archivos.map((file, idx) => (
                                <li key={idx} className="flex justify-between items-center py-1 px-2 hover:bg-gray-100">
                                    <span className="cursor-pointer hover:underline" onClick={() => abrirModal(file)}
                                    >
                                        📄 {file.nombre}
                                    </span>
                            ...
                            <div className="flex flex-col items-end mt-2 pr-2 text-sm space-y-1">
                                ...
                                <div className="text-right">
                                    Subtotal: $ {productosLocal
                                        .reduce((acc, p) => acc + p.precio_unitario * p.cantidad, 0)
                                        .toFixed(2)}
                                </div>
                                {/* Descuento total */}
                                <div className="text-right">
                                    Descuento total: $ {productosLocal
                                        .reduce(
                                            (acc, p) => acc + p.precio_unitario * p.cantidad * (p.porcentaje_descuento / 100), 0
                                        )
                                        .toFixed(2)}
                                </div>
                                ...
                                {/* Total <div className='text-lg font-semibold text-right'> Total: $ {productosLocal.reduce((acc, p) => acc + p.importe, 0).toFixed(2)} </div>*/}
                            </div>
                        </TabsContent>
                    ...
                    <div className="flex justify-between items-center mt-6">
                        ...
                        <div className="flex gap-3">
                            ...
                            <Button className="bg-green-700 hover:bg-green-800 text-white" disabled={!camposCompletos} onClick={handleSubmit}
                            >
                                Guardar Factura
                            </Button>
                        </div>
                        ...
                        <>
                            {archivoSeleccionado.file ? (
                                ...
                                archivoSeleccionado.file.type.startsWith('image/') ? (
                                    <img src={URL.createObjectURL(archivoSeleccionado.file)} alt={archivoSeleccionado.nombre} className="max-h-96 w-auto mx-auto" />
                                ) : archivoSeleccionado.file.type === 'application/pdf' ? (
                                    <iframe src={URL.createObjectURL(archivoSeleccionado.file)} className="w-full h-96" />
                                ) : (
                                ...
                                archivoSeleccionado.mime.startsWith('image/') ? (
                                    <img src={archivoSeleccionado.url} alt={archivoSeleccionado.nombre} className="max-h-96 w-auto mx-auto" />
                                ) : archivoSeleccionado.mime === 'application/pdf' ? (
                                    <iframe src={archivoSeleccionado.url} className="w-full h-96" />
                                ) : (
                                    <p className="mt-4 text-gray-500">No se puede previsualizar este tipo de archivo.</p>
                                )
                            )}
