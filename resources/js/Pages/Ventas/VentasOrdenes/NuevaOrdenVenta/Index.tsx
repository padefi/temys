...
type PageProps = InertiaPageProps & {
  ...
  impuestos: Impuesto[]
  ordenVentaElegida?: OrdenesVenta
  flash?: {
    success?: string;
    danger?: string;
  };
...
export default function Index() {
    ...
    const ordenVenta = ordenVentaElegida;
    const [busqueda, setBusqueda] = useState( ordenVenta?.cliente?.nombre || '')
    const [mostrarLista, setMostrarLista] = useState(false)

    const [entrega_esperada, setEntregaEsperada] = useState(ordenVenta?.entrega_esperada ? new Date(ordenVenta?.entrega_esperada).toISOString().slice(0,10) : '')

    const [monedaId, setMonedaId] = useState<number | null>(ordenVenta?.tipo_moneda?.id || null);
    const [monedaNombre, setMonedaNombre] = useState(
    ordenVenta?.tipo_moneda
        ? `${ordenVenta.tipo_moneda.descripcion} - ${ordenVenta.tipo_moneda.simbolo}`
        : ''
    );
    const [monedaCodigo, setMonedaCodigo] = useState<string | null>(ordenVenta?.tipo_moneda?.codigo || null);
    const [monedaInvalida, setMonedaInvalida] = useState(false)

    const [almacenId, setAlmacenId] = useState<number | null>(ordenVenta?.almacen?.id || null);
    const [almacenNombre, setAlmacenNombre] = useState(
    ordenVenta?.almacen
        ? `${ordenVenta.almacen.nombre} - ${ordenVenta.almacen.tipo}`
        : ''
    );

    const [almacenInvalida, setAlmacenInvalida] = useState(false)

    const [observaciones, setObservaciones] = useState(ordenVenta?.observaciones || '')
    ...
    const [estadoOrden, setEstadoOrden] = useState(ordenVenta?.estado || '');
    ...
    ordenVenta?.cotizacion_moneda || null
    );
    ...
    const archivosOrdenVenta: Archivo[] = (ordenVenta.archivos ?? []).map(a => ({
    id: a.id,
    nombre: a.nombre, // <--- corregido
    url: `/ventas/ordenes-ventas/archivo/${a.id}`,
    mime: a.mime,
    size: a.size,
    isCotizacion: false
    }));

    setArchivos([...archivosCotizacion, ...archivosOrdenVenta]);
    }, [ordenVenta]);
  ...
  const cancelarOrdenVenta = () => {
    if (!ordenVenta?.id) return;

    router.post(
        '/ventas/ordenes-ventas/cancelar',
        { ordenVenta: ordenVenta.id },
        {
        onSuccess: () => {
    ...
    /////////////Confirmar Orden Venta
    const confirmarOrdenVenta= () => {

    router.post('/ventas/ordenes-ventas/confirmar', {
      ordenVenta: ordenVenta?.id,
    ...
    ///////////Guardar Orden Venta
    const guardar = () => {
      router.post('/ventas/ordenes-ventas/guardar', {
        ordenVenta: ordenVenta?.id,
        cliente: busqueda,
        moneda: monedaId,
        cotizacion_moneda: monedaCodigo !== "ARS" ? cotizacionMoneda : null,
    ...
    const handleUploadFile = async (ordenId: number, archivo: File) => {
        ...
        router.post(`/ventas/ordenes-ventas/${ordenId}/archivo`, formData, {
        forceFormData: true,
    ...
    const handleGenerarFactura = (data: any) => {
        router.reload({ only: ['ordenVentaElegida', 'flash'] }) // 🔁 recarga parcial
    }

return (
    <AuthenticatedLayout header={<h2 className="text-xl font-semibold">Nueva Orden de Venta</h2>}>
      <Head title="Nueva Orden de Venta"></Head>
    ...
    <div className="py-12">
        ...
        <Button
            variant="outline"
            size="lg"
            onClick={() => router.visit('/ventas/ordenes-ventas')}
        >
            Volver
        </Button>
         <Button
            variant="outline"
            size="lg"
            disabled={!formularioCompleto || ordenVenta?.estado === 'Cancelada' || ordenVenta?.estado === 'Finalizada' || estaBloqueada }
            onClick={() => guardar()}
        >
            Guardar
        </Button>
        ...
        <div className="items-end">
            <div className="flex flex-col items-end">
                ...
                {ordenVenta?.created_at && (
                <div className="text-sm text-gray-500">
                    {new Date(ordenVenta?.created_at || '').toLocaleDateString('es-AR', {
                        day: '2-digit',
                        month: '2-digit',
                        year: 'numeric',
                        hour: '2-digit',
               ...
               {estadoOrden && (
                <div>
                    Orden de Venta N° {ordenVenta?.id}
                    <br></br>
                    Estado : {estadoOrden}
                </div>
                )}
          ...
          <div className="mt-4">
            <CargaProductos
                ...
                estadoOrden={ordenVenta?.estado ?? "Pendiente"}
            />
        ...
        <div className="mt-4">
            ...
            <li key={idx} className="flex justify-between items-center py-1 px-2 hover:bg-gray-100">
                ...
                {!file.isCotizacion && ordenVenta?.estado == 'Pendiente' && (
                <AlertDialog>
                    ...
                    <AlertDialogFooter>
                        ...
                        onClick={() => {
                            ...
                            if (file.id) {
                            router.post(`/ventas/ordenes-ventas/archivo/${file.id}/eliminar`, {}, {
          ...
          <div className="flex gap-4 mt-6 justify-end">
                <Button
                onClick={confirmarOrdenVenta}
                disabled={!formularioCompleto || (ordenVenta?.estado === 'Confirmada' || ordenVenta?.estado === 'Cancelada')}
                >
                ...
                variant="outline"
                onClick={cancelarOrdenVenta} disabled={!formularioCompleto || (ordenVenta?.estado === 'Confirmada' || ordenVenta?.estado === 'Cancelada')}
                >
                    Cancelar
                </Button>
                ...
                disabled={!formularioCompleto || (ordenVenta?.estado !== 'Confirmada')}
                >
                    Cargar Factura
                </Button>
            ...
            <div className="mt-6">
                ...
                {ordenVenta.comprobantes_clientes.map((f) => (
                    ...
                    {f.archivos?.length > 0 && (
                        ...
                        variant="outline"
                        onClick={() =>
                            window.open(`/ventas/facturas-clientes/${f.id}`, '_blank')
                        }
                        >
                        Ver
                        </Button>
    ...
    <GenerarFacturaModal
            ...
            setProductos={setProductos}
            tipoMonedas={tipoMonedas}
            clienteId={ordenVenta?.cliente?.id || 0}
