...
export default function Index() {
    ...
    const [motivosND, setMotivosND] = useState<any[]>([])
    const [motivoNDId, setMotivoNDId] = useState<string>("")
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        // Mapear detalles de factura → items ND
        ...
        // Si hay factura, por defecto ND total
        setTipo("total")

    }, [facturaId])

    // Obtener motivos ND cuando cargue la página
    useEffect(() => {
        fetch('/contabilidad/motivos-nota-debito')
        .then(res => res.json())
        .then(data => setMotivosND(data))
    }, [])
    ...
    //TOTAL DE LA NOTA DE DEBITO
    const totalNotasDebito = (factura: any) =>
    ...
    const notasDebito = totalNotasDebito(factura)

    return total - pagos - notasDebito
    }
    //VALIDAR ND VS SALDO
    const excedeSaldo =
    tipo === "parcial" &&
    ...
    const payload = {
        ...
        motivo_nota_debito_id: motivoNDId,
    ...
    try {
        ...
        '/contabilidad/notas-debito',
        payload
        )

        toast.success("Nota de débito emitida correctamente");
        setItems([])
        setObservacion("")
        setTipo("total")
        setCondicionVentaId("")
        ...
        alert('Ocurrió un error al emitir la nota de débito')
        }
    }
    }
    ...
    return (
        ...
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800">Notas de Débito</h2>}>
        <Head title="Notas de Débito" />

        <div className="max-w-6xl mx-auto space-y-6 mt-6">
            <h1 className="text-2xl font-semibold">Nota de Débito – Cliente</h1>
            ...
            <Card>
                ...
                <div>
                    <label className="text-sm">Fecha Nota Débito</label>
                    <Input
                    type="date"
                    value={fecha}
                ...
                <label className="text-sm">Motivo de la Nota de Débito</label>
                <Combobox
                    value={motivoNDId}
                    onValueChange={setMotivoNDId}
                    placeholder="Seleccionar motivo"
                    options={motivosND.map(m => ({
                    label: m.descripcion,
                    value: m.id.toString()
                    }))}
                />
        ...
        <Card>
            <CardContent className="pt-6 space-y-4">
                ...
                <div className="grid grid-cols-14 gap-2 text-xs font-medium text-muted-foreground border-b pb-2">
                    ...
                    <div className="col-span-1 text-left">Importe ND</div>
