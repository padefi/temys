...
type Cobro = {
  ...
  emisorBancoId?: number
  depositoCuentaId?: number
  numeroCheque?: string
  tarjetaId?: number
}
...
type Props = {
  ...
  clienteId: number
  facturasSeleccionadas: { id: number; monedaOrden: number; total: number; montoACobrar: number }[]
...
export default function GenerarOrdenCobroModal({
  ...
  facturasDetalle,
    ...
    const [metodoCobros, setMetodoCobros] = useState<MetodoTesoreria[]>([]);
...
console.log()
    ...
    axios
        .get(`/contabilidad/cliente/${clienteId}/anticipos-disponibles`)
    ...
    //////////TRAE METODO COBROS
    const fetchMetodoCobros = async () => {
        try {
            const res = await axios.get("/metodo-cobros");
            const data = Array.isArray(res.data) ? res.data : res.data.data;
            setMetodoCobros(Array.isArray(data) ? data : []);
        } catch (error) {
            console.error("Error al traer los métodos de cobro", error);
            setMetodoCobros([]);
        }
    };
    if (open) fetchMetodoCobros();
    }, [open]);
    ...
    useEffect(() => {
        ...
        const newCuotas: Cobro[][] = Array.from({ length: cuotas }, (_, i) =>
            cobrosCuotas[i] && cobrosCuotas[i].length
            ? cobrosCuotas[i]
    ...
    const totalFacturas = facturasSeleccionadas.reduce(
        (acc, f) => acc + toNumber(f.montoACobrar),
        0
    )

    const totalACobrarNeto = round2(Math.max(totalFacturas - totalAnticipos, 0))
    const esSoloAnticipos = totalACobrarNeto === 0
    ...
    const calcTotal = (cobros: Cobro[]) =>
    cobros.reduce((acc, p) => acc + toNumber(p.importe), 0)
    ...
    const handleAddCobroCuota = (cuotaIdx: number) => {
        ...
        const faltante = getImporteFaltante(totalCobrosCuotas)
  ...
  const handleFinalSubmit = async () => {
    ...
    const totalCobros = plan === "unico" ? round2(totalCobrosUnicos) : round2(totalCobrosCuotas)
    const faltante = getImporteFaltante(totalCobros)
    ...
    if (faltante < 0) {
      toast.error("El total de cobros supera el total a pagar.")
      return
    }*/

    if (totalCobros < totalACobrarNeto) {
      toast.error("Falta completar el monto a cobrar")
      return
    }

    if (totalCobros > totalACobrarNeto) {
      toast.error("El total supera el monto a cobrar")
      return
    }
...
try{
    const dataToSend =
        ...
        ? {
            ...
            cobros: cobrosUnicos.map((p) => ({
                ...
                banco_emisor_id: p.emisorBancoId ? Number(p.emisorBancoId) : null, // ✅ entero o null
                banco_deposito_id: p.depositoBancoId ? Number(p.depositoBancoId) : null, // ✅ entero o null
                cuenta_deposito_id: p.depositoCuentaId ? Number(p.depositoCuentaId) : null,
    ...
    const res = await axios.post("/contabilidad/ordenesTesoreriaClientes", dataToSend)
    if (res.status === 201) {
        toast.success("Orden de cobro generada exitosamente")
  ...
  const renderCamposCondicionales = (
    ...
    return (
      <>
        ...
        {showError && p.metodo === "Transferencia" }
        ...
        {p.metodo === "Transferencia" && (
          <div className="col-span-6 grid grid-cols-1 gap-2">
            <CampoBancoTarjeta
              tipo="transferencia"
              value={{
                depositoBancoId: p.depositoBancoId,
                depositoCuentaId: p.depositoCuentaId ? Number(p.depositoCuentaId) : undefined,
                emisorBancoId: p.emisorBancoId ? Number(p.emisorBancoId) : undefined,
              }}
              onChange={(val) => updateMultipleFields({ depositoBancoId: val.depositoBancoId, depositoCuentaId: val.depositoCuentaId, emisorBancoId: val.emisorBancoId })}
            />
        ...
        {p.metodo === "Cheque" && (
           <div className="col-span-6">
                      ...
                      value={{
                            depositoBancoId: p.depositoBancoId,
                            depositoCuentaId: p.depositoCuentaId,
                            emisorBancoId: p.emisorBancoId,
                            fechaCheque: p.fechaCheque,
                            numeroCheque: p.numeroCheque,
                            importeOrden: p.importe
                      }}
                      onChange={(val) =>
                          updateMultipleFields({
                            emisorBancoId: val.emisorBancoId,
                            depositoBancoId: val.depositoBancoId,
                            depositoCuentaId: val.depositoCuentaId,
                            fechaCheque: val.fechaCheque,
                            numeroCheque: val.numeroCheque
                          })
                      }
                      />
            {showError && !p.emisorBancoId && <p className="text-sm text-red-600 mt-1">Seleccioná un banco emisor</p>}
            </div>
        )}
    ...
    const isCobroCompleto = (p: Cobro) => {
            console.log(p.emisorBancoId, p.depositoBancoId, p.depositoCuentaId, p.fechaCheque, p.numeroCheque)
        const camposBasicos = p.metodo_id > 0 && p.moneda > 0 && p.importe > 0 && !!p.fecha
            if (!camposBasicos) return false

            if (p.metodo === "Cheque")
            return !!p.emisorBancoId
                && !!p.depositoBancoId
                && !!p.depositoCuentaId
                && !!p.fechaCheque
                && !!p.numeroCheque
            if (p.metodo === "Tarjeta") return !!p.tarjetaId
            if (p.metodo === "Transferencia") return !!p.emisorBancoId && !!p.depositoBancoId && !!p.depositoCuentaId
            return true
    }
    const totalCobrosActual =
        plan === "unico"
            ? round2(totalCobrosUnicos)
            : round2(totalCobrosCuotas)

    const coincideTotal =
        round2(totalCobrosActual) === round2(totalACobrarNeto)

    const saldoRestante = round2(totalACobrarNeto - totalCobrosActual)

    const cobrosCompletos =
        plan === "unico"
            ? cobrosUnicos.length > 0 && cobrosUnicos.every(isCobroCompleto)
            : cobrosCuotas.length > 0 &&
            cobrosCuotas.every(
                (cuota) => cuota.length > 0 && cuota.every(isCobroCompleto)
            )

    const isFormValid = cobrosCompletos && coincideTotal
                ...
                {cobrosUnicos.map((p, idx) => (
                  <div key={idx} className="grid grid-cols-6 gap-2 mb-2 items-center">
                    ...
                    onChange={(e) => {
                        ...
                        const selectedMetodo = metodoCobros.find((m) => m.id === selectedId);
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                {cobrosCuotas.map((cobroList, cuotaIdx) => (
                  <TabsContent key={cuotaIdx} value={String(cuotaIdx)}>
                    ...
                    <div className="text-sm text-gray-600 mt-2">Total de esta cuota: {formatCurrency(calcTotal(cobrosCuotas[cuotaIdx]), tipoMonedas[0]?.codigo)}</div>
