...
export default function Index() {
    ...
    const { proveedores,impuestos } = usePage().props as any;
    ...
    const [motivosReembolso, setMotivosReembolso] = useState<any[]>([])
    const [motivoReembolsoId, setMotivoReembolsoId] = useState<string>("")
  ...
  useEffect(() => {
    ...
    fetch(`/contabilidad/proveedores/${proveedorId}/facturas`)
        .then(res => res.json())
        .then(data => {
        setFacturas(data)
        })
    ...
    useEffect(() => {
        fetch('/contabilidad/motivos-reembolso')
        .then(res => res.json())
        .then(data => setMotivosReembolso(data))
    }, [])
    ...
    //TOTAL DE LOS REEMBOLSOS
    const totalReembolso = (factura: any) =>
    ...
    const reembolso = totalReembolso(factura)

    return total - pagos - reembolso
    }
    //VALIDAR REEMBOLSO VS SALDO
    const excedeSaldo =
    tipo === "parcial" &&
    facturaSeleccionada &&
    ...
    const payload = {
        ...
        motivo_reembolso_id: motivoReembolsoId,
    ...
    try {
        ...
        toast.success("Reembolso emitido correctamente");
        setItems([])
        setObservacion("")
        setTipo("total")
        setCondicionVentaId("")
        setMotivoReembolsoId("")
        setFecha("")
        setFechaVencimiento("")
        setFacturaId("")
        setProveedorId("")
    ...
    return (
        ...
        <div className="max-w-6xl mx-auto space-y-6 mt-6">
            <h1 className="text-2xl font-semibold">Reembolso Proveedor</h1>
            ...
            <Card>
                ...
                <Combobox
                    ...
                    onValueChange={(val) => {
                    setProveedorId(val)
                    setFacturaId("") // resetea factura al cambiar proveedor
                    }}
                    placeholder="Seleccionar proveedor"
                    options={proveedores.map((p: any) => ({
                    label: p.padron.cuit
                        ? `${p.razon_social} - ${p.nombre_fantasia} - ${p.padron.cuit}`
                ...
                <div>
                    ...
                    <Combobox
                        ...
                        disabled={!proveedorId || loadingFacturas}
                    />
                </div>
                ...
                <Combobox
                    value={motivoReembolsoId}
                    onValueChange={setMotivoReembolsoId}
                    placeholder="Seleccionar motivo"
                    options={motivosReembolso.map(m => ({
                    label: m.descripcion,
                    value: m.id.toString()
                    }))}
                />
        ...
        <Card>
            <CardContent className="pt-6 space-y-4">
                ...
                <div className="text-sm text-muted-foreground">
                    ...
                    Fecha {formatFechaAR(facturaSeleccionada.fecha_factura)} ·
        ...
        <Card>
            ...
            <div className="text-xl font-semibold">
                Total Reembolso: ${total.toFixed(2)}
            </div>
            </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-2">
            ...
            <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded p-3">
                ⚠️ El importe del reembolso (${total.toFixed(2)}) debe ser menor al de la factura (${saldoPendiente(facturaSeleccionada).toFixed(2)}).
            </div>
            )}
