...
export default function Index() {
  const { ordenesTesoreria, clientes, metodosCobro, monedas, tiposComprobante, bancos, cuentasBancarias, tarjetas } = usePage().props as any;
  ...
  const requiereBanco = (metodoId: number) => {
    const metodo = metodosCobro.find((m: any) => m.id === metodoId);
  ...
  const ordenesFiltradas = useMemo(() => {
    return ordenesActualizadas.filter((o: any) => {
      ...
      if (filter.tipo_id && comprobantes[0]?.tipo_id != filter.tipo_id) return false;
  ...
  const guardarCambios = async () => {
    ...
    try {
        ...
        ordenes: seleccionadas.map(id => {
            ...
            cbu_cobro: orden.cbu_cobro,
            importe: orden.importe,
            tipo_id: orden.comprobantes?.[0]?.tipo_id ?? null,
  ...
  return (
    <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800">Órdenes de Cobro</h2>}>
      <Head title="Órdenes de Cobro" />
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50">
              ...
              <TableHead>
                <FilterCombobox
                  items={clientes.map((p: any) => ({ value: String(p.id), label: p.nombre }))}
                  value={filter.tipo_id}
                  onChange={v => handleChange('tipo_id', v)}
              ...
              <TableHead>
                <FilterCombobox
                  items={metodosCobro.map((m: any) => ({ value: String(m.id), label: m.nombre }))}
                  value={filter.metodo_id}
                  onChange={v => handleChange('metodo_id', v)}
                  placeholder="Método Cobro"
                  emptyLabel="Sin resultados"
                />
              </TableHead>
              ...
              return (
                ...
                <TableCell>
                    <input
                        ...
                        // Condiciones base: mismo cliente y método de cobro
                        const mismoCliente =
                        ...
                        const metodo = metodosCobro.find((m: any) => m.id === o.metodo_id);
                        ...
                        if (esTransferencia) {
                            ...
                            mismoOrigen = o.cbu_cobro === primera.cbu_cobro;
                        }
                        ...
                        if (!(mismoCliente && mismoMetodo && mismoOrigen)) {
                            toast.error("Solo se pueden seleccionar órdenes del mismo cliente, método y origen de cobro");
                            return;
                        }
                    ...
                    <TableCell>{comprobantes?.[0]?.cliente?.apellido + " - " + comprobantes?.[0]?.cliente?.nombre || "-"}</TableCell>
                    ...
                    <Input
                        placeholder="CBU"
                        value={cambios.cbu_cobro ?? o.cbu_cobro ?? ''}
                        onChange={(e) => handleEdit(o.id, 'cbu_cobro', e.target.value)}
                        className="w-[23ch]"
                    />
                    </TableCell>
                    ...
                    <TableCell>{o.fecha_cobro}</TableCell>
                  </TableRow>
